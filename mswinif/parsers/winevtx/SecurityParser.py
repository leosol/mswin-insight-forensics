    ...
    def can_handle(self, filename):
        if filename.endswith('.evtx'):
            if "security.evtx" == filename.strip().lower():
                return True
            if "Archive-Security" in filename:
                return True
        return False
    ...
    def process(self, filepath):
        ...
        for node, err in self.xml_records(filepath):
            ...
            if 4624 == event_id or 4647 == event_id or 4648 == event_id or 4625 == event_id or 4634 == event_id or 4768 == event_id or 4769 == event_id or 4776 == event_id:
                ...
                insert_dict["package_name"] = data_dict.get("PackageName", "")
                insert_dict["service_name"] = data_dict.get("ServiceName", "")
                insert_dict["service_sid"] = data_dict.get("ServiceSid", "")
                insert_dict["ticket_options"] = data_dict.get("TicketOptions", "")
                insert_dict["ticket_encryption_type"] = data_dict.get("TicketEncryptionType", "")
                insert_dict["transmitted_services"] = data_dict.get("TransmittedServices", "")
